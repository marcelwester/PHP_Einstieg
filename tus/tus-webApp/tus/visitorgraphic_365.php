...
switch ($action) {
   case "tus":
		$sqlstr="select date_format(toc,'%Y-%m-%d') x,hits y from sys_counter1 where toc>sysdate()-INTERVAL 1 YEAR   order by x";
		
		$sqlstr1="select min(date_format(toc,'%Y-%m-%d')) min_date,max(date_format(toc,'%Y-%m-%d')) max_date from sys_counter1 where toc>sysdate()-INTERVAL 1 YEAR";
...
// Überschrift
imagettftext ($im,$fontsize +4 ,0,150,30,$text_color,$font,"Besucher-Statistik des letzten Jahres");
