...
<?php
      sitename("ma_tabelle_popup.php",$_SESSION["groupid"]);
	  $teamid = $_REQUEST["teamid"];
	  $saisonid = $_REQUEST["saisonid"];
	  
	  
	
	  $sqlstr = "select ma.name,sa.spielzeit,sa.liga from fb_saison sa,fb_mannschaft ma 
	  ...
	  $sqlstr1 = "select name,id from fb_mannschaft ma, fb_zwtab_mannschaft_saison zw where 
	  ...
	  echo '<TABLE WIDTH="100%" BORDER="0" >';
          ...
          $sqlstr = "select id,spieltag,DATE_FORMAT(datum,'%d.%m.%Y') datum,datum datum_sort,
                     TIME_FORMAT(datum,'%H:%i') uhrzeit,heim_id,aus_id,spieltag,heim_tore,aus_tore,gespielt
                     from fb_spiele where saison_id=$saisonid and (heim_id=$teamid or aus_id=$teamid) order by datum_sort";
          $result = GetResult($db,$sqlstr);
	  ...
	  unset($result1);
          ...
          echo '<TABLE WIDTH="100%" BORDER="0" >';
          	echo '<TR><TD ALIGN="CENTER" BGCOLOR="#DDDDDD"><B>Datum</B></TD><TD ALIGN="CENTER" BGCOLOR="#DDDDDD"><B>Spieltag</B></TD><TD ALIGN="CENTER" BGCOLOR="#DDDDDD"><B>Info</B></TD><TD ALIGN="CENTER" BGCOLOR="#DDDDDD"><B>Heim</B></TD><TD ALIGN="CENTER" BGCOLOR="#DDDDDD"><B>Gast</B></TD><TD ALIGN="CENTER" BGCOLOR="#DDDDDD"><B>Ergebnis</B></TD></TR>';
                foreach ($result as $match)
                {
                	...
                	echo '<td ALIGN="LEFT">';
                		//echo $match["datum"].' - '.$match["uhrzeit"];
          				echo date("d.m.Y - H:i",strtotime($match["datum_sort"]))." (".convert_weekday(date("D",strtotime($match["datum_sort"]))).")";
                	echo '</td>';         
			...
			echo '<td align="CENTER">';
                                echo '<a href="ma_spielplan_info_popup.php?descent=self&teamid='.$teamid.'&spielid='.$match["id"].'">';
