...
sitename("ma_kontakt.php",$_SESSION["groupid"]);
$url="index.php?site=team&action=kontakt&teamid=$teamid&saisonid=$saisonid&PHPSESSID=".session_id();
...
case show:
        ...
        if (priv("kontakt") && priv_team($teamid))
	{
	...
	$sqlstr="select kontakt from fb_tus_mannschaft where id=$teamid";
...
case 'save':
	if (priv("kontakt") && priv_team($teamid))
	{
		...
		$state=$_POST["kontakt"];
		$sql = "update fb_tus_mannschaft set kontakt ='".$state."' where id=".$teamid;
		$result = doSQL($db, $sql);

		if ($result["code"] == 0)
		{
			mlog("Fussball: Kontakt einer Fussballmannschaft wurde gespeichert: ".$teamid);
