...
function getEinzel ($personid,$saisonid,$tusid,$pk) {
	...
	foreach ($result as $spiel) {
		...
		$sqlstr .= $nonocholt."1_id<>0 and ";
		$sqlstr .= "spiel_id=".$spiel["id"]." and ";
		$sqlstr .= "heim_saetze>-1 and aus_saetze>-1 and ";
		$sqlstr .= "spiel_nr in (".$pk.")";
		$result1=getResult($db,$sqlstr);
		if (isset($result1)) {
	...
	return array("siege" => $siege, "niederlagen" => $niederlagen,
		     "saetzePlus" => $saetzeGewonnen, "saetzeMinus" => $saetzeVerloren);
}
...
sitename("tt_mannschaft.php",$_SESSION["groupid"]);
		...
		// Falls image_id in saison ungleich 0 ist iwrd das foto anstelle von $bild_id genommen
		if ($result_saison["0"]["image_id"]!="0") 
			$bild_id=$result_saison["0"]["image_id"];
...
//		Mannschaftsauswahl
		...
		echo 'Mannschaft : <SELECT onChange="window.location.href=\''.$url.'&ttmenu=yes&teamid=\'+this.value;">';
		$sql = 'select * from tt_tus_mannschaft order by reihenfolge asc';
		$result = GetResult($db, $sql);
...
//		Saisonauswahl
        ...
        echo 'Saison : <SELECT onChange="window.location.href=\''.$url.'&ttmenu=yes&saisonid=\'+this.value;">';
...
switch($action)
{
		...
		<!--
    		function info(spielerid)
        	{
                	var url;
        	<?php
                	echo 'url = "tt_ma_kader_info_popup.php?action=info&personid="+spielerid+"&PHPSESSID='.session_id().'";';
        	?>
