		...
		if ($result["0"]["anz"]==16) {
			...
			// Prüfen der Aufstellung: Es wüssen jeweils mindestens 6 Spieler vorhanden sein
			
			if ((!isset($resultheim["5"]["spieler_id"]) || !isset($resultaus["5"]["spieler_id"])) && !isset($_POST["force"])) {
				echo "<br>Überprüfen Sie die Aufstellung. Es müssen beim ersten Aufruf dieser Seite von jeder Mannschaft mind. 6 Spieler aufgestellt sein.";
			...
			$sqlstr = $pre_sqlstr.$spielid.",4,".convert_null($resultheim["0"]["spieler_id"]).",".convert_null($resultaus["1"]["spieler_id"]).")";
			$result=doSQL($db,$sqlstr);
			$sqlstr = $pre_sqlstr.$spielid.",5,".convert_null($resultheim["1"]["spieler_id"]).",".convert_null($resultaus["0"]["spieler_id"]).")";
			$result=doSQL($db,$sqlstr);
			$sqlstr = $pre_sqlstr.$spielid.",6,".convert_null($resultheim["2"]["spieler_id"]).",".convert_null($resultaus["3"]["spieler_id"]).")";
			$result=doSQL($db,$sqlstr);
			$sqlstr = $pre_sqlstr.$spielid.",7,".convert_null($resultheim["3"]["spieler_id"]).",".convert_null($resultaus["2"]["spieler_id"]).")";
			$result=doSQL($db,$sqlstr);
			...
			$result=doSQL($db,$sqlstr);
			$sqlstr = $pre_sqlstr.$spielid.",10,".convert_null($resultheim["0"]["spieler_id"]).",".convert_null($resultaus["0"]["spieler_id"]).")";
			$result=doSQL($db,$sqlstr);
			$sqlstr = $pre_sqlstr.$spielid.",11,".convert_null($resultheim["1"]["spieler_id"]).",".convert_null($resultaus["1"]["spieler_id"]).")";
			$result=doSQL($db,$sqlstr);
			$sqlstr = $pre_sqlstr.$spielid.",12,".convert_null($resultheim["2"]["spieler_id"]).",".convert_null($resultaus["2"]["spieler_id"]).")";
			$result=doSQL($db,$sqlstr);
			$sqlstr = $pre_sqlstr.$spielid.",13,".convert_null($resultheim["3"]["spieler_id"]).",".convert_null($resultaus["3"]["spieler_id"]).")";
			$result=doSQL($db,$sqlstr);
			...
			// Letzte Doppelbegegnung (Wenn bis dahin keine Entscheidung gefallen ist)
			$sqlstr  = "insert into tt_spiele_saetze (spiel_id,spiel_nr,heim1_id,heim2_id,aus1_id,aus2_id) values (";
			$sqlstr .= $spielid.",";
			$sqlstr .= "16,0,0,0,0)";
			$result=doSQL($db,$sqlstr);
