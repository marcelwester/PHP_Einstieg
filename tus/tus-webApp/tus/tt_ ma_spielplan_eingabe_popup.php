<?php
//// tt_ma_spielplan_eingabe_popup.php
////
...
switch ($_REQUEST["action"])
{
 case 'add':
     ...
     $sqlstr="select max(spieltag) from tt_spiele where saison_id=".$saisonid;
	...
	if ($exit==0) {
	     for ($i=1; $i<=10; $i++) {	
      		if (($_POST["ausid".$i] != "-1") && ($_POST["heimid".$i] != "-1") && $exit==0) {
      			...
      			$zeit=$_POST["zeit".$i];
      			$ts=ts2db($datum,$zeit);
      			$sqlstr='insert into tt_spiele (datum,saison_id,spieltag,heim_id,aus_id,gespielt) values ('
