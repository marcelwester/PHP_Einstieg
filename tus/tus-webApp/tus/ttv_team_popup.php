...
include "inc.php";
sitename("ttv_team_popup.php",$_SESSION["groupid"]);
?>
...
{
	case 'add':
	 echo '<BR><FORM METHOD="POST" ACTION="ttv_team_popup.php?action=save&PHPSESSID='.session_id().'">';
		...
		break;
	case 'edit':
		$sql = 'select * from tt_mannschaft where id = '.$_REQUEST["teamid"];
	...
	case 'save':
		...
		if (strlen($_POST["name"]) != 0)
		{
			...
			{
				$sql = 'insert into tt_mannschaft (name, vereinsnummer, internet,kat) ';
			...
			mlog("Tischtennisverwaltung: Speichern einer Mannschaft: ".$id);
			if ($result["code"] == 0)
			{
