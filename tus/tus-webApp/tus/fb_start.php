...
// news.php
//
// letzte Änderung : Daniel, 28.03.2004
// - Datei erstellt
//
...
<?php
		...
		echo '<TABLE WIDTH="100%" BORDER="0">';
			echo '<TR>';
				...
				echo '</TD>';
				echo '<TD VALIGN="TOP" BGCOLOR="#DDDDDD">';
					echo '<B>zufälliger Sponsor</B>';
				echo '</TD>';
			echo '</TR>';

			echo '<TR>';
				echo '<TD VALIGN="TOP" CLASS="eins">';
					$sql = 'select sp.id, datum , heim_id, aus_id from fb_spiele sp,fb_saison sa';
					$sql .= ' where sp.datum > now() and gespielt=0  and (heim_id in ('.$tusteams.') or aus_id in ('.$tusteams.'))';
					$sql .= ' and saison_id=sa.id and sa.turnier=0';
					$sql .= ' order by datum asc limit 10';
					$result = getResult($db,$sql);
					if (isset($result[0]))
					{
				...
				echo '</TD>';
				echo '<TD CLASS="eins" VALIGN="CENTER">';
			...
			echo '<TR>';
				...
				echo '<TD VALIGN="TOP" CLASS="eins">';
					...
					$sql.= 'where sb.spiel_id = sp.id and gespielt=1 order by sp.datum desc limit 8';
					$result = getResult($db,$sql);
					if (isset($result[0]))
					{
						$height = count($result) * 26;
