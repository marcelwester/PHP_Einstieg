...
////
//// fbv_saison.php
//// letzte Änderung : Volker, 10.04.2005 
//// was : Auswahl der Anzeige von aktuellen und geschlossenen Saisons
...
if (priv("fb_saison"))
{
sitename("fbv_saison.php",$_SESSION["groupid"]);
?>
...
<!--
	function popup(saisonid, action)
	{
		...
		if (saisonid == 0)
<?php
			echo 'url = "fbv_saison_popup.php?action="+action+"&PHPSESSID='.session_id().'";';
		echo ' else ';
			echo 'url = "fbv_saison_popup.php?action="+action+"&saisonid="+saisonid+"&PHPSESSID='.session_id().'";';
?>
		if (action == "teams")
			var heigth = 500;
		else
...
<?php
	...
	$_url = 'index.php?site=fbverw&action=saison&PHPSESSID='.session_id();
...
*/
			...
			$sql = 'select * from fb_saison where closed=0 order by kat,startdatum desc, kat desc';
			...
			foreach ($result as $saison)
			{
		        echo '<TR>';
	                ...
	                echo '</TD>';
	                echo '<TD ALIGN="CENTER" BGCOLOR="#FFFFFF">';
	                        $sqlstr1='select name from fb_tus_mannschaft,fb_zwtab_mannschaft_saison where id=mannschaft_id and saison_id='.$saison["id"]." order by name";
...
*/
			...
			if ($closed == "1") {
				...
				$sql = 'select * from fb_saison where closed=1 order by kat,startdatum desc, kat desc';
				$result = getResult($db, $sql);
				foreach ($result as $saison)
				{
			        echo '<TR>';
		                ...
		                echo '</TD>';
		                echo '<TD ALIGN="CENTER" BGCOLOR="#FFFFFF">';
		                        $sqlstr1='select name from fb_tus_mannschaft,fb_zwtab_mannschaft_saison where id=mannschaft_id and saison_id='.$saison["id"];
		                        $result1=GetResult($db,$sqlstr1);
		                        if (isset($result1)) 
		                        	echo $result1["0"]["name"];
		                        else
