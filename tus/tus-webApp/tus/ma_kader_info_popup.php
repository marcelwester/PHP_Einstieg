...
session_start();
include "inc.php";
sitename("ma_kader_info_popup.php",$_SESSION["groupid"]);
?>
...
/*            echo '<TR>';
           ...
           echo '</TR>';
            ...
            echo '<TR>';
				...
				$sqlstr  = "select archive_image from fb_zwtab_person_typ_tus_mannschaft where ";
	...
	// Statistik
	$sqlstr="select curdate() datum,id,hits,hits_current_day,DATE_FORMAT(toc,'%Y-%m-%d') toc from sys_counter_spielerinfo where sportart='fb' and id=".$_REQUEST["personid"];
	$result=GetResult($db,$sqlstr);
	//print_r($result);
	if (! isset($result["0"]["id"])) {
		$sqlstr="insert into sys_counter_spielerinfo (id,hits,hits_current_day,toc,sportart) values (".$_REQUEST["personid"].",1,1,sysdate(),'fb')";
	} else {
		$hits=$result["0"]["hits"];
		$hits++;
		...
		$sqlstr="update sys_counter_spielerinfo set hits=".$hits.",hits_current_day=".$hits_current.",toc=sysdate() where id=".$_REQUEST["personid"]." and sportart='fb'";
	}
	$dummy=doSQL($db,$sqlstr);
