...
include "inc.php";
sitename("fbv_team_tus_popup.php",$_SESSION["groupid"]);
focus();
?>
...
switch ($_REQUEST["action"])
{
	case 'add':
		...
		echo '<BR><FORM METHOD="POST" ACTION="fbv_team_tus_popup.php?action=save&PHPSESSID='.session_id().'">';
		echo '<TABLE WIDTH="100%" BORDER="0">';
			...
			echo '<TD ALIGN="LEFT" BGCOLOR="#FFFFFF">';
				...
				echo '<TD ALIGN="CENTER" BGCOLOR="#FFFFFF">';
					$url = 'fbv_team_tus_popup.php?action=add&PHPSESSID='.session_id();
					echo '<SELECT onChange="window.location.href=\''.$url.'&kat=\'+this.value;">';
						$sql = 'select kategorie,name from fb_mannschaft_kat order by name';
			...
			echo '<TR>';
				...
				echo '</TD>';
                    echo '<TD ALIGN="CENTER" BGCOLOR="#FFFFFF">';
                            ...
                            $sql_teams = 'select id, name from fb_mannschaft 
            ...
            echo '<TR>';
                    ...
                    echo '</TD>';
                    echo '<TD ALIGN="CENTER" BGCOLOR="#FFFFFF">';
                            $sql_fotos = 'select image_id,descr from sys_images where kategorie = 1 and linked = 0';
	...
	case 'edit':
		...
		echo '<TABLE WIDTH="100%" BORDER="0">';
			...
			echo '<TR>';
				...
				echo '</TD>';
                    echo '<TD ALIGN="CENTER" BGCOLOR="#FFFFFF">';
                            $result_kat=GetResult($db,"select kat from fb_mannschaft where id=".$_REQUEST["teamid"]);
                            $sql_teams = "select id, name from fb_mannschaft where kat='".$result_kat["0"]["kat"]."'";
                            $result_teams = getResult($db,$sql_teams);
                            echo '<select name="team">';
                            echo '<option value="0" selected>keine</option>';
		...
		echo '</TR>';
	       	...
	       	echo '</TD>'; 
	       	echo '<TD ALIGN="CENTER" BGCOLOR="#FFFFFF">'; 
	       	$sqlstr="select id,spielzeit,liga from fb_saison,fb_zwtab_mannschaft_saison 
	...
	case 'save':
		if (isset($_POST["id"]))
			...
			if (!isset($id))	// neu anlegen
			{
			    if ($team != 0)
                            {                     
			        $sql = "insert into fb_tus_mannschaft (id,name,bild,reihenfolge,show_menu,spielstaette_id) 
			...
			{
				$sql = 'update fb_tus_mannschaft set ';
				$sql .= 'name = "'.$name.'", ';
				$sql .= 'id ='.$team.', ';
				$sql .= 'reihenfolge='.$idx.', ';
			...
			if ($result["code"] == 0)
			{
				mlog("Fussballverwaltung: Es wurde eine TuS Mannschaft gespeichert: ".$id);
