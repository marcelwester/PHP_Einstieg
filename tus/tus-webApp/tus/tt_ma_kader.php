...
//// Berücksichtiugung der Saisonid beim Lesen aus tt_zwtab_person_typ ....
...
?>
		window.open(url,"spieler","width=650, height=400, top=200, left=200, menubar=no, directories=no, resizeable=yes, toolbar=no, scrollbars=yes, status=no");
	}
...
<?php
			...
			echo 'url = "tt_ma_kader_print_popup.php?saisonid="+saisonid+"&teamid="+teamid+"&PHPSESSID='.session_id().'";';
?>
...
?>
        function popup(spielerid)
        {
              var url;
              <?php
                echo 'url = "tt_ma_spieler.php?action=start&teamid='.$teamid.'&saisonid='.$saisonid.'&PHPSESSID='.session_id().'";';
              ?>
...
<?php
sitename("tt_ma_kader.php",$_SESSION["groupid"]);
        ...
        if (priv("kader") && priv_tt_team($teamid))
	{
        	$show_edit="1";
                        ...
                        echo '<TR>';
                                ...
                                echo '<TD ALIGN="CENTER" BGCOLOR="#FFFFFF">';
                                        echo '<a href="javascript:popup(0);">';
                                        echo '<B>Kader / Aufstellung editieren</B>';
                                        echo '</a>';
                                echo '</TD>';
                        echo '</TR>';
        ...
        if ((priv("kader") ||priv("kader_print")) && priv_tt_team($teamid))
        {
                       echo '<TR>';
                                echo '<TD ALIGN="CENTER" BGCOLOR="#FFFFFF">';
	...
	echo '</TABLE></CENTER><BR>';
        ...
        echo '<CENTER><TABLE WIDTH="70%" BORDER="0">';
                echo '<TR>';
                        echo '<TD ALIGN="CENTER" BGCOLOR="#DDDDDD" width="5%">';
                                echo '<B>Pos</B>';
                        echo '</TD>';
                ...
                echo '</TR>';
                        ...
                        foreach ($result_typ as $typ)
                        {
                                ...
                                $sql_pers = 'select pe.name,pe.vorname,zw.person_id,zw.position
                                ...
                                if (isset($result_pers[0])) {
 	                                echo '<TR><TD ALIGN="LEFT" COLSPAN="3"'.$colspan_typ.'" BGCOLOR="#FFFFFF"><B>'.$typ["name"].'</B></TD></TR>';
