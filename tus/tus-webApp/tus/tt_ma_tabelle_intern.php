...
<!--
        ...
        function tabelle_statistik(teamid)
        {
        	var url;
        	<?php
        	   echo 'url = "tt_ma_tabellenstatistik_popup.php?teamid="+teamid+"&saisonid='.$saisonid.'&PHPSESSID='.session_id().'";';	
        	?>
...
//// Tabelle anlegen
////////////////////////
sitename("tt_ma_tabelle_intern.php",$_SESSION["groupid"]);
    $tabelle = array();
...
////////////////////////
    ...
    $sql3 = 'select * from tt_spiele where saison_id = '.$saisonid.' and gespielt = 1';;
    ...
    foreach ($result3 as $row3) // tt_spiele
    {
            ...
            while ($y < $anzteams)
            {
                    if ($tabelle[$y]["id"] == $row3["heim_id"])                ////////// Heimmannschaft
                    {
                            ...
                            if ($row3["heim_tore"] < $row3["aus_tore"]) {
                            	    // Niederlage
                                    $tabelle[$y]["niederlagen"]++;
                                    $tabelle[$y]["minuspunkte"] += 2;
                            }

                            if ($row3["heim_tore"] == $row3["aus_tore"]) {
                            	 // Unentschieden
                                 $tabelle[$y]["punkte"]++;
                                 $tabelle[$y]["minuspunkte"] += 1;
                            }
                    }
...
////////////////////////
    ...
    do
    {
            ...
            while ($y < $anzteams-1)
            {
                    $punktecurrent = ($tabelle[$y]["punkte"]) - ($tabelle[$y]["minuspunkte"]);
                    $punktenext = ($tabelle[$y+1]["punkte"]) - ($tabelle[$y+1]["minuspunkte"]);
                    
                    //if ($tabelle[$y]["punkte"] < $tabelle[$y+1]["punkte"])
                    if ($punktecurrent < $punktenext) 
                    {
                            $dummy = $tabelle[$y];
    ...
    echo '<TABLE WIDTH="100%" BORDER="0">';
            ...
            foreach($tabelle as $team)
            {
                    echo '<TR>';
                            ...
                            echo '</TD>';
                            echo '<TD ALIGN="CENTER" BGCOLOR="#FFFFFF">';
                                    echo $team["punkte"].':'.$team["minuspunkte"];
                            echo '</TD>';
                            echo '<TD ALIGN="CENTER" BGCOLOR="#FFFFFF">';
