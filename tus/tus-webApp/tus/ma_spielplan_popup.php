...
include "inc.php";
sitename("ma_spielplan_popup.php",$_SESSION["groupid"]);
focus();
?>
...
switch ($_REQUEST["action"])
{
 case 'add':
     ...
     echo '<FORM method="post" action="'.$PHP_SELF.'">';
       ...
       // Datum und Uhrzeit
           echo '<TD>Datum </TD><TD><INPUT type=text name=datum size=14 value="'.date('d.m.Y').'"></TD>';
           //echo '<BR></BR>';
           echo '<BR><TD>Uhrzeit </TD><TD><INPUT type=text name=zeit size=14 value="00:00"></TD></BR>';
           // <TD>Uhrzeit </TD><TD><INPUT type=text name=uhrzeit size=14 value="00:00:00">
 ...
 case 'save_add':
      ...
      if ($result["code"] == 0)
      {
        if ( ($austore == "") || ($heimtore == "") )
        {
          ...
          $sqlstr="insert into fb_spiele (saison_id,heim_id,aus_id,datum,spieltag,userid,toc,gespielt)
          values  ($saisonid,$heimid,$ausid,'$ts',$spieltag,$userid,now(),0)";

        }
        else
        {
          // Spielergebnis wurde eingetragen
          $sqlstr="insert into fb_spiele (saison_id,heim_id,aus_id,heim_tore,aus_tore,datum,spieltag,userid,toc,gespielt)
          values  ($saisonid,$heimid,$ausid,$heimtore,$austore,'$ts',$spieltag,$userid,now(),1)";
        }
        //echo "<br>".$sqlstr;
 ...
 case 'edit':
       ...
       // Letzte Änderung lesen (name,toc)
          ...
          $sqlstr = "select su.name,$TS from fb_spiele sp,sys_users su where sp.userid=su.userid and sp.id=$spielid";
          $result = GetResult($db, $sqlstr);
       ...
       // Heimmannschaft
          ...
          $sql = "select sp.spielstaette_id,sp.bemerkung,ma.id,ma.name,heim_tore,aus_tore,$TS,spieltag,gespielt
                  from fb_mannschaft ma,fb_spiele sp
       ...
       //Gastmannschaft
          $sql = "select ma.id,ma.name from fb_mannschaft ma,fb_spiele sp where aus_id=ma.id and sp.id=$spielid";
          $result = GetResult($db, $sql);
          $ausid=$result[0]["id"];
       ...
       // Pruefen, ob eine TuS - Mannschaft beteiligt ist:
          $sqlstr = "select count(*) tus from fb_tus_mannschaft where ";
      ...
      echo '<TABLE WIDTH="100%" BORDER="0" >';
      echo '<TR><TD COLSPAN="3" ALIGN="CENTER" BGCOLOR="#DDDDDD">';
      echo '<B>Spieldaten editieren</B><BR>'.$liga.' - '.$spielzeit.'<BR> - '.$username.' - </TD><TR>';
      echo '</TABLE>';

      // Spieltag
         echo "<B>Spieltag: </B>";
         $sql = "select distinct(spieltag) from fb_spiele where saison_id=".$saisonid." order by spieltag ";
         $result=GetResult($db,$sql);
         build_select($result,"spieltag","spieltag","tag","","1",$spieltag);
         echo "<br></br>";
      // Datum
         echo '<b>Datum: </b><INPUT type=text name=datum size=14 value="'.$datum.'"/>';
      ...
      // Spielstätte
         ...
         echo '<BR> <BR> ';
         echo '<b>Uhrzeit: </b> <TD><INPUT type=text name=zeit size=14 value="'.$zeit.'"/>';
         echo '<BR>';
         if ($gespielt == "1")
         {
           echo '<BR><INPUT type=radio name=gespielt value=0 >noch nicht gespielt     ';
           echo '<INPUT type=radio name=gespielt value=1 checked>gespielt</BR>';
         }
         else
         {
           echo '<BR><INPUT type=radio name=gespielt value=0 checked>noch nicht gespielt';
           echo '<INPUT type=radio name=gespielt value=1>gespielt</BR>';
         }

// Bilder
        $sql_fotos = 'select image_id, descr, linked from sys_images where kategorie = 3 and (linked = 0 or linked = '.$spielid.') order by descr';
        $result_fotos = getResult($db,$sql_fotos);

         echo '<br><b>Verfügbare Bilder';
      ...
      // Spielbericht lesen
         $sql="select date,spiel_id,name,spiel_bericht,edit_id from fb_sp_bericht,sys_users su where spiel_id=$spielid and verfasser_id=su.userid";
         $result=GetResult($db,$sql);
         if (isset($result)) {
...
// Bilder
    $sql_fotos = 'update sys_images set linked = 0 where kategorie = 3 and linked = '.$spielid;
    $result_fotos = doSQL($db,$sql_fotos);

    $sql_fotos = 'select image_id from sys_images where kategorie = 3 and linked = 0 order by descr';
    $result_fotos = getResult($db,$sql_fotos);
    ...
    if (isset($result_fotos))
    {
      ...
      if ($result["code"] == 0)
      {
        $sqlstr="update fb_spiele set heim_id=$heimid,aus_id=$ausid,heim_tore=$heimtore,
                 aus_tore=$austore,spieltag=$spieltag,datum='$ts',userid=$userid,toc=now(),
                 gespielt=$gespielt,bemerkung='$bemerkung',spielstaette_id=$spielstaetteid
                where id=$spielid";
        //echo $sqlstr;
        $result=doSQL($db,$sqlstr);
        ...
        if (! isset($spielberichtid)) {
          //$sqlstr="delete from fb_sp_bericht where spiel_id=$spielid";
          //$result=doSQL($db,$sqlstr);
          //$state = addcslashes($spielbericht,'",\'');
          $state = $spielbericht;
          $sqlstr="insert into fb_sp_bericht (spiel_bericht,spiel_id,verfasser_id, date) values
                   ('$state',$spielid,$userid,now())";
        } else {
          $sqlstr  = "update fb_sp_bericht set ";
          $sqlstr .="spiel_bericht='$spielbericht',";
          //$sqlstr .="spiel_bericht='".addcslashes($spielbericht,'",\'')."',";
          $sqlstr .="edit_id=".$userid.",";
          $sqlstr .="date=sysdate() ";
          $sqlstr .="where spiel_id=".$spielid;
        }
          $result=doSQL($db,$sqlstr);
          mlog("Fussball: Speichern eines Spielberichtes: ".$spielid);
          //echo $sqlstr;
        }
        else
        {
      ...
      if ($result["code"] == 0)
      {
           mlog("Fussball: Speichern eines Fussballspiels: ".$spielid);
           echo 'Die Aktion wurde erfolgreich ausgeführt.<br><A HREF="javascript:opener.location.reload(); window.close();">Hier klicken, um das Fenster zu schließen</A>';
           echo '<SCRIPT TYPE="text/javascript">';
                echo 'opener.location.reload();';
                echo 'setTimeout("window.close()",1000);';
