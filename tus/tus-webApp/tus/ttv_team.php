...
if (priv("tt_team"))
{
sitename("ttv_team.php",$_SESSION["groupid"]);
?>
...
<!--
	function popup(teamid,kat)
	{
		...
		if (teamid == 0)
<?php
			echo 'url = "ttv_team_popup.php?action=add&kat="+kat+"&PHPSESSID='.session_id().'";';
		echo ' else ';
			echo 'url = "ttv_team_popup.php?action=edit&teamid="+teamid+"&PHPSESSID='.session_id().'";';
?>
...
<?php
	...
	$sql = 'select kat from tt_mannschaft group by kat';
	$result_kat = getResult($db, $sql);

	$katSelect = '<SELECT NAME="KAT" ID="KAT" onChange="window.location.href=\'index.php?site=ttverw&action=team&PHPSESSID='.session_id().'&kat=\'+this.value;">';
	foreach($result_kat as $katrow)
		if ($katrow["kat"] == $kat)
			$katSelect .= '<OPTION SELECTED VALUE="'.$katrow["kat"].'">'.$katrow["kat"].'</OPTION>';
		else
			$katSelect .= '<OPTION VALUE="'.$katrow["kat"].'">'.$katrow["kat"].'</OPTION>';
	$katSelect .= '</SELECT>';
