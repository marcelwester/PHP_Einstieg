...
include "inc.php";
sitename("ma_spielplan_eingabe_popup.php",$_SESSION["groupid"]);
?>
...
switch ($_REQUEST["action"])
{
 case 'add':
     ...
     $sqlstr="select max(spieltag) from fb_spiele where saison_id=".$saisonid;
     $result=GetResult($db,$sqlstr);
     if (isset($result)) {
     	$spieltag=$result["0"]["max(spieltag)"];
     	$spieltag++;
     } else {
     	$spieltag=1;
     }
     ...
     echo '<FORM method="post" action="'.$PHP_SELF.'">';
       ...
       // Spieltag
          echo "<B>Spieltag: </B>";
          echo '<INPUT TYPE="NUMBER" SIZE="2" NAME="tag" VALUE="'.$spieltag.'" />';
 ...
 case 'save':
      ...
      // Daten Validieren	
      if (preg_match("/\D/", $spieltag) || $spieltag=="") {
      	echo "<br><b>Eingabefehler Spieltag </b>".$spieltag;
      	$exit=1;
      }
      if ($exit==0) {
      	for ($i=1; $i<=10; $i++) {
	...
	if ($exit==0) {
	     for ($i=1; $i<=10; $i++) {	
      		if (($_POST["ausid".$i] != "-1") && ($_POST["heimid".$i] != "-1") && $exit==0) {
      			...
      			$zeit=$_POST["zeit".$i];
      			$ts=ts2db($datum,$zeit);
      			$sqlstr='insert into fb_spiele (datum,saison_id,spieltag,heim_id,aus_id,gespielt) values ('
      				.$ts.','.$saisonid.','.$spieltag.','
      				.$_POST["heimid".$i].','
      				.$_POST["ausid".$i].','
      				.'0)';
      ...
      if ($exit == 0)
      {
		   mlog("Fussball: Es wurde ein Spieltag neu eingetragen: ".$saisonid);	
