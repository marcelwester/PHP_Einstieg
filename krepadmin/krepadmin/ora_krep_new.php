...
$url='index.php?menu=newsetup&PHPSESSID='.session_id();
		...
		echo '<form method="POST" action="'.$url.'&action=step1">';
         echo '<tr>';
		      echo '<td align="center" width="50%">';
		   	   echo '<b>zu replizierendes Schema:</b>';
	   	   echo '</td>';
	 		   echo '<td align="center" width="50%">';
	         	$sqlstr="select username from all_users order by username";
	         	$result=getResult($db,$sqlstr);
	         	build_select($result,"USERNAME","USERNAME","owner");
	         	unset($result);
	         echo '</td>';
	      echo '</tr>';
...
/*
	      echo '<tr>';
	         echo '<td align="center" width="50%">';
	            echo '<b>Replikationspropagator:</b>';
	         echo '</td>';
	         echo '<td align="center" width="50%">';
	            $sqlstr="select username from all_users order by username";
	            $result=getResult($db,$sqlstr);
	            build_select($result,"USERNAME","USERNAME","propagator");
	            unset($result);
	         echo '</td>';
	      echo '</tr>';
*/
   ...
   case 'step1':
	// Checking for Database link
   	$owner=$_POST["owner"];
      $propagator=$_SESSION["repadmin"];
	   echo '<table align="center" width="50%" border="1">';
		echo '<form method="POST" action="'.$url.'&action=step2">';
         echo '<tr>';
		      echo '<td align="center" width="50%">';
		   	   echo '<b>zu replizierendes Schema:</b>';
	   	   echo '</td>';
	      ...
	      echo '<tr>';
	         echo '<td align="center" width="50%">';
	            echo '<b>Datenbanklink:</b>';
	         echo '</td>';
	         echo '<td align="center" width="50%">';
	            $sqlstr  ="select object_name from dba_objects where ";
	            $sqlstr .="OBJECT_TYPE='DATABASE LINK' and ";
	            $sqlstr .="status='VALID' and ";
	            $sqlstr .="owner='".$propagator."' order by object_name";
               $result=getResult($db,$sqlstr);
               build_select($result,"OBJECT_NAME","OBJECT_NAME","dblink");
	         echo '</td>';
	      echo '</tr>';
   ...
   case 'step2':
	// TableSelection
      ...
      $dblink=$_POST["dblink"];
	   echo '<table align="center" width="50%" border="1">';
		echo '<form method="POST" action="'.$url.'&action=step3">';
         echo '<tr>';
		      echo '<td align="center" width="50%">';
		   	   echo '<b>zu replizierendes Schema:</b>';
	   	   echo '</td>';
	      ...
	      echo '<tr>';
	         ...
	         echo '</td>';
	         echo '<td align="center" width="50%">';
					$sqlstr="select table_name from all_tables where OWNER='".$owner."' order by table_name";
					$result=getResult($db,$sqlstr);
...
/*
		// Tabellen aus der Replikation lesen
      $sqlstr="select ONAME from dba_repobject where type='TABLE'";
      $result=getResult($db,$sqlstr);
		$i=0;
      unset($tables);
      $tables=array();
